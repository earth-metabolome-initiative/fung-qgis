#!/usr/bin/env python3
"""Fetch species observed in the Neuchatel/Swiss fung collection context."""
...
# Neuchatel area in EPSG:4326. Country-wide Swiss workflows normally use --country-code CH.
DEFAULT_SWLAT = 46.93
DEFAULT_SWLNG = 6.82
DEFAULT_NELAT = 47.05
DEFAULT_NELNG = 7.04
...
def resolve_taxon_id(
    taxon_name: str,
    *,
    lookup_taxa: Callable[..., dict[str, Any]] = get_taxa_autocomplete,
) -> tuple[int, str]:
    response = lookup_taxa(q=taxon_name, per_page=10)
    results = response.get("results", [])
    normalized_name = taxon_name.casefold()
    exact_matches = [
        taxon
        for taxon in results
        if str(taxon.get("name", "")).casefold() == normalized_name
        or str(taxon.get("matched_term", "")).casefold() == normalized_name
        or str(taxon.get("preferred_common_name", "")).casefold() == normalized_name
    ]
    candidates = exact_matches or results
    if not candidates:
        raise argparse.ArgumentTypeError(f"Could not resolve taxon name {taxon_name!r}")
    taxon = candidates[0]
    return int(taxon["id"]), str(taxon.get("name") or taxon_name)
...
def resolve_search_taxon(args: argparse.Namespace) -> None:
    if args.taxon_id is not None and args.taxon_name is not None:
        raise argparse.ArgumentTypeError("Use only one of --taxon-id or --taxon-name")
    if args.taxon_id is not None:
        args.resolved_taxon_id = args.taxon_id
        args.resolved_taxon_name = ""
        return
    if args.taxon_name is not None:
        taxon_id, taxon_name = resolve_taxon_id(args.taxon_name)
        args.resolved_taxon_id = taxon_id
        args.resolved_taxon_name = taxon_name
        return
    args.resolved_taxon_id = None
    args.resolved_taxon_name = ""
...
def taxon_count_to_row(
    item: dict[str, Any],
    *,
    query_taxon_id: int | None,
    query_taxon_name: str,
) -> dict[str, str]:
    ...
    return {
        ...
        "query_taxon_id": normalize_text(query_taxon_id),
    ...
    *,
    ...
    while True:
        ...
        if taxon_id is not None:
            location_params["taxon_id"] = taxon_id
        response = fetch_page(
            **location_params,
...
def write_metadata(path: Path, args: argparse.Namespace, row_count: int) -> None:
    ...
    lines = [
        "iNaturalist species counts for the fung collection project",
        ...
        f"taxon_id={args.resolved_taxon_id or ''}",
        f"taxon_name={args.resolved_taxon_name or ''}",
        f"per_page={args.per_page}",
        f"max_pages={args.max_pages or ''}",
        f"row_count={row_count}",
        "note=The default query uses a Neuchatel bounding box. Country queries use "
        "iNaturalist place IDs. Taxon-name queries are resolved through iNaturalist.",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch species-level iNaturalist observation counts for the fung "
            "collection project and write a CSV collection-list candidate file."
        )
    )
    parser.add_argument(
        "--bbox",
    ...
    parser.add_argument(
        "--country-code",
        default=None,
        help="ISO 3166-1 alpha-2 or alpha-3 country code, e.g. CH or CHE for Switzerland.",
    )
    parser.add_argument(
        "--taxon-id",
        type=int,
        default=None,
        help="iNaturalist taxon ID whose descendant observed species should be counted.",
    )
    parser.add_argument(
        "--taxon-name",
        default=None,
        help="iNaturalist taxon name to resolve and use as --taxon-id.",
    )
    parser.add_argument(
        "--iconic-taxa",
        nargs="+",
...
def main(argv: list[str] | None = None) -> int:
    ...
    rows = fetch_species_counts(
        ...
        taxon_id=args.resolved_taxon_id,
        taxon_name=args.resolved_taxon_name,
